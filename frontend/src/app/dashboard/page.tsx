      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-600">
              ...
              <code className="rounded-md bg-stone-200/60 px-1.5 py-0.5 text-xs text-stone-700">
                POST /ingest/equity_snapshot
              </code>{" "}
              on the API).
            </p>
          </div>
        ...
        {!report ? (
          <div className="rounded-xl border border-stone-200 bg-[var(--cl-card)] p-5 text-sm text-stone-700 shadow-sm">
            We couldn&apos;t load{" "}
            <span className="rounded bg-stone-100 px-1 font-medium text-stone-800">{api}/equity/report</span>. Start
            the FastAPI server and seed the database when you&apos;re ready.
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-stone-200 bg-[var(--cl-card)] p-4 shadow-sm">
              ...
              <p className="mt-2 text-3xl font-semibold text-stone-800">{report.summary.equity_score ?? "—"}</p>
